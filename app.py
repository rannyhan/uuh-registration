...
def init_sheet():
    try:
        ...
        if sheet.row_count == 0 or sheet.cell(1, 1).value != 'id':
            sheet.clear()
            sheet.append_row(['id', 'name', 'org', 'phone', 'email', 'round', 'dept', 'question', 'created_at'])
...
def api_register():
    ...
    if not all([name, org, phone, email, round_, dept]):
    ...
    try:
        ...
        sheet = get_sheet()
        sheet.append_row([new_id, name, org, phone, email, round_, dept, question, now])
...
def api_admin_export():
    ...
    try:
        ...
        writer.writerow(['번호', '이름', '소속', '연락처', '이메일', '차수', '부서', '질의사항', '신청일시'])
